...
public class AccountDAO {
    ...
    public void insertAccount(Account acc) throws SQLException {
        ...
        String sql = "INSERT INTO account (id_acc, email, password, role, status) VALUES (?, ?, ?, ?, ?)";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    account.setId(generateAccountId());        // id_acc thay cho userId
    ...
    public boolean insertgg(Account account) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            ps.setString(5, account.getGoogle_id());
